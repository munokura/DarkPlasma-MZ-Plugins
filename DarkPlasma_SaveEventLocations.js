...
@plugindesc Record event location and orientation on a map
@author DarkPlasma
@license MIT
...
version: 1.0.0
In RPG Maker MZ, event positions and orientations are initialized when a map
is loaded.
This plugin records the event positions and orientations on the map,
and restores them based on that information when the map is loaded again.

Writing the following in the map's memo field
will record the positions and orientations of all events on that map.
<saveEventLocations>

Writing the following in the event's memo field
will record the position and orientation of that event.
<saveLocation>

The recorded positions and orientations will be included in the save data.
The more events you record,
the larger the save data size will be.
*/
