// DarkPlasma_NotifyWishList 1.0.0
...
@plugindesc Wishlist Notifications
@author DarkPlasma
@license MIT
...
version: 1.0.0
Notifies you when you have all the materials for an item on your wishlist.

Notifications will not be sent for recently notified items.

Recently notified items will be excluded under the following conditions:
- Loading a save game
- The item is removed from your wishlist

This plugin requires the following plugins:
DarkPlasma_StoreWishList version: 1.0.0
TorigoyaMZ_NotifyMessage version: 1.6.1
If using with the following plugins, add this plugin below them:
DarkPlasma_StoreWishList
...
@param message
@desc 通知する文章を設定します。{item}が成果物アイテムの名前に置換されます。
@text 通知文
@type string
...
(() => {
  ...
  function isSameKindItem(data1, data2) {
    return (
      (DataManager.isItem(data1) && DataManager.isItem(data2)) ||
  ...
  const settings = {
    message: String(pluginParameters.message || `{item}の素材が揃いました。`),
  };

  function Game_Temp_NotifyWishListMixIn(gameTemp) {
    ...
    gameTemp.removeRecentNotifiedWishListItem = function (item) {
      ...
      this._recentNotifiedWishListItems = this._recentNotifiedWishListItems.filter(
        (i) => i.id !== item.id || !isSameKindItem(i, item),
      );
    };
    ...
    gameWishList.notifyCompletionByGainMaterial = function (material) {
      ...
      targets
        ...
        .forEach((target) => {
          ...
          if (data) {
            $gameTemp.reserveWishListNotification(
  ...
  function Game_Party_NotifyWishListMixIn(gameParty) {
  ...
  function Scene_NotifyWishListMixIn(sceneClass) {
  ...
  function Scene_CanNotifyWishListMixIn(sceneClass) {
    sceneClass.canNotifyWishListItem = function () {
  ...
  Torigoya.NotifyMessage.parameter.advancedAppendScenes
    ...
    .forEach((scene) => Scene_CanNotifyWishListMixIn(window[scene].prototype));
})();
