...
@plugindesc A status effect that is only effective on targets whose level is a multiple of a specific value.
@author DarkPlasma
@license MIT
...
version: 1.0.2
Adds a state effect to the skill or item effect that is only active on targets
whose level is a multiple of a specific number.

Write the following in the memo field of the target skill or item:
<levelXState:
5:1
>
This will inflict State 1 (Incapacitated) on targets whose level is a multiple
of 5.

This state effect will be disabled if the target's state effectiveness is 0 or
if it has the state disable flag.
However, it will always be active in all other cases.

This state effect will be disabled if the target has no level.
Enemy characters do not inherently have levels, but you can enable this effect
on enemy characters by using a plugin that assigns levels to enemy characters.
