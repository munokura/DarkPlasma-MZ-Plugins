...
@plugindesc Equipment filtering function
@author DarkPlasma
@license MIT
...
Version: 1.5.1
Provides a filtering function based on equipment traits.

Press the shift key while selecting equipment to enter filtering mode.

Interface for External Plugins
Scene_Equip.prototype.equipFilterTraitWindowRect: () => Rectangle
Rectangle of the filtering trait display window
...
Filter Data Builder
Adds rules for generating filtering data (described below).
See the following plugin for specific usage examples.
...
Adds a rule to extract trait data from equipment for filtering.
Use this when you want to filter custom-defined traits.

EquipFilterBuilder.prototype.withTraitToEffectNameRule
: ((traitId: number, dataId: number) => string|null) => EquipFilterBuilder
Adds a rule to return the effect name from a specified trait ID and effect ID.
Use this when you want to filter custom-defined effects.

EquipFilterBuilder.prototype.withoutTrait: (number) => EquipFilterBuilder
Specify trait IDs to exclude from display.

EquipFilterBuilder.prototype.withTrait: (number) => EquipFilterBuilder
Add trait IDs to display.
Use this when you want to filter custom-defined traits.
...
If using with the following plugins, add this above.
...
@param traitName
@text Feature Name
@desc Set the special ability value feature name to be displayed on the filter screen.
@type struct<TraitName>

@param selectedItemColor
@text Filter Color
@desc Set the color of the filter ON items.
@type color
@default 2

@param key
@text Filtering Key
@desc Set the key to perform the filtering operation.
@type select
@default shift
@option shift
@option menu
