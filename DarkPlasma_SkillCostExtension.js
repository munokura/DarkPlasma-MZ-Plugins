...
@plugindesc Extending skill costs
@author DarkPlasma
@license MIT
...
Version: 1.3.4
You can add costs to a skill's memo field by writing the following:

<SkillCost:
hp: (Fixed HP Cost)
hpRate: (Percentage of Maximum HP Cost)
mpRate: (Percentage of Maximum MP Cost)
item: (Item ID): (Quantity)
gold: (Money)
variable: (Variable ID): (Numeric Value)
>

You can specify only one item.
You can also specify multiple items for item and variable.

Example 1: A skill that consumes one item ID1 and one item ID2.
...
Example 2: A skill that consumes 5 points for variable ID1 and 1 point for
variable ID3.
<SkillCost:
variable:1:5
variable:3:1
...
About Cost Display in the Skill List
This plugin does not display expanded costs for items or money.
If you want to display the extension cost,
use an additional plugin such as DarkPlasma_SkillCostExtensionView.

Regarding the function that reflects consumption in the item count when
selecting an item
This plugin only reflects the number of items consumed by skills.
To reflect the number of items consumed when using an item directly,
use DarkPlasma_ConsumeItemImmediately.
When using this plugin in conjunction with this plugin,
place this plugin below DarkPlasma_ConsumeItemImmediately.

When using with the following plugins,
place them below them:
DarkPlasma_ConsumeItemImmediately

@param consumeImmediately
@text Consumption reflected when input
@desc When selecting a skill that consumes items or gold, the number of items/gold that subsequent members refer to will be the one after consumption.
@type boolean
@default true
*/
